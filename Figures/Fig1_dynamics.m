...
subplot(3,1,1); %Corresponding to panel A, no interventions
hold on
...
subplot(3,1,2); %Corresponding to panel B, Mitigation
...
subplot(3,1,3); %Corresponding to panel C, Prevention
% figure(2)
hold all
%plot S
patch([T_vec(steps) fliplr(T_vec(steps))],...
    [S_prew_mean(steps)+S_prew_std(steps) fliplr(S_rew_mean(steps)-S_prew_std(steps))],...
    color(1,:),'FaceAlpha',0.1,'EdgeColor','none','HandleVisibility','off');
